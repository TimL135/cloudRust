...
export async function encryptFileForMultipleUsers(
  ...
  console.log(`🔐 Verschlüssele Datei "${file.name}" (${file.size} bytes)...`);
  ...
  console.log(`✅ Datei verschlüsselt (${encryptedFile.encryptedData.length} chars Base64)`);
  ...
  for (const recipient of recipients) {
    ...
    console.log(`🔑 Key verschlüsselt für User: ${recipient.userId}`);
  }
...
async function addRecipientToEncryptedFile(
  ...
  console.log(`➕ Füge neuen User "${newRecipient.userId}" hinzu...`);
